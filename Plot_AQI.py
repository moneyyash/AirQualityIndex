...
def avg_data_2013():
    temp_i = 0
    average =[]
    for rows in pd.read_csv('Data/AQI/aqi2013.csv', chunksize = 24): # chunksize mean it takes 24 rows at a time ,
                                                                     #  we here take 24 to calculate the avg of one day
        add_var = 0
        avg = 0.0
        data=[]
        ...
        for i in data:
            if type(i) is float or type(i) is int:
                ...
                if i!='NoData' and i!='PwrFail' and i!='---' and i!='InVid': # check if data is present but in string format
                    temp = float(i)                                           # NoData , PwrFail,---, InVld means no data is present 
                    add_var+=temp
    ...
    for rows in pd.read_csv('Data/AQI/aqi2014.csv', chunksize = 24):
        ...
        for i in data:
            if type(i) is float or type(i) is int:
                ...
                if i!='NoData' and i!='PwrFail' and i!='---' and i!='InVid':
                    temp = float(i)
                    add_var+=temp
